...
def test_Exp(epoch, loader, net, best_acc, step=0):
    ...
    with torch.no_grad():
        ...
        Exp_F1, Exp_acc, Exp_F1_per_class = metric_for_Exp(tmp_Exp_label, tmp_Exp_pred,8)
        results = {'pred':tmp_Exp_pred, 'label':tmp_Exp_label, 'prob':tmp_Exp_prob, 'out':tmp_out}


        final_Exp_score = 1 * Exp_F1

        print(f"epoch: {epoch},step:{step},Exp_best_score:{final_Exp_score},Exp_F1: {Exp_F1} Exp_acc: {Exp_acc}")
        print(
            "epoch: {}, Loss: {}, Exp_F1: {} Exp_acc: {}  Exp_score: {} \n".format(
                epoch, Exp_loss_sum / len(loader),
                Exp_F1, Exp_acc, final_Exp_score))
        Exp_F1_per_class = [f'{k:.06f}' for k in Exp_F1_per_class]
...
if __name__ == '__main__':
    ...
    Exp_trainset = ABAW2_Exp_data("./annos/ABAW3_new_exp_training.csv", img_path, emb_dict,mfcc_dict,word_dict,
                                  coff_dict,None,60,60,60,60,transform1)
    trainset = Exp_trainset
    Exp_testset = ABAW2_Exp_data("./annos/test_unseen_highconf.csv", img_path, emb_dict,mfcc_dict,
                                 word_dict,coff_dict,None,60,60,60,60,transform1)
    ...
    trainloader = data.DataLoader(trainset, batch_size=bz, num_workers=16,shuffle=False)
    ...
    # ck_save_path = './checkpoints/single_task/baseline_EXP:CE+baseline+Single_exp_detect_transconcat_mfcc_seq60_words_seq60_embs_seq60_coff_seq60'
    ckpt_path = f'{ck_save_path}/Exp_best.pth'
    ckpt_path = './checkpoints/single_task/fusion_061823.pth'
    ...
    # best_Exp_score = train_Exp(0, trainloader, net, optimizer, best_Exp_score)
    results = test_Exp(0, Exp_testloader, net, best_Exp_score)
    # best_Exp_score = test_Exp(0, trainloader, net, best_Exp_score)
    save_pkl = ck_save_path.split('/')[-1]+'.pkl'
    with open(f'./results/{save_pkl}', 'wb') as f:
